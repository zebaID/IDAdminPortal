App.controller('searchedJobReportCtrl', ['$scope', '$rootScope', '$filter', 'ngTableParams', '$resource', '$timeout', //ngTableDataService,
   ...
   $rootScope.getUserforSelectedCity = function(city){
        ...
        $rootScope.getDriverJobReport = function() {
            
            $rootScope.loader = 1;
            $rootScope.fetchJobData = [];
            var allJobData = [];
             
              var fromDate = $localStorage.get('searchReportFromDate');
             var toDate = $localStorage.get('searchReportToDate');
            var untime = Math.round((new Date(fromDate)).getTime() / 1000);
            var untime2 = Math.round((new Date(toDate)).getTime() / 1000);
            if($rootScope.roleId === '1'){
                 if(angular.isUndefined($rootScope.operationCitySelect) || $rootScope.operationCitySelect === null){
                    ...
                    if($rootScope.operationCitySelect === 'All'){
                           //all
                              DriverJobDetails.getDriverJobReport({
                    fromDate:fromDate,
                    ToDate:toDate,
